...
public class generate_bunny : MonoBehaviour {
	...
	{
		if (start_time - (nb_rabbit * 0.3F + nb_trap * 0.4F + time * 0.005F) < 5)/*If time is smaller than 5s*/
			yield return new WaitForSeconds(5);
		else
			yield return new WaitForSeconds(start_time - (nb_rabbit * 0.3F + nb_trap * 0.4F + time * 0.005F));
